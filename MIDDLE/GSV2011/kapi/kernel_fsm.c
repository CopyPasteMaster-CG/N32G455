...
void AvFsmFunPlugTxFsm(GSV_CONFIG *gsv_config,AvPort *port)
{
    ...
    switch (*port->content.is_PlugTxFsm)
    {
      case AvFsmPlugTxDefault:
        *port->content.is_PlugTxFsm = AvFsmPlugTxReset;
        KfunPtVarInit(gsv_config,port);
        KfunPtResetPort(gsv_config,port);
        KfunTxSetMuteAv(gsv_config,port);
        break;
        ...
        else if (port->content.tx->EdidReadSuccess == AV_EDID_FAIL)
        {
            if (tx->IgnoreEdidError == 0)
            {
                *port->content.is_PlugTxFsm = AvFsmPlugTxEdidError;
                KfunTxEdidError(gsv_config,port);
            }
      ...
      case AvFsmPlugTxEdidManage:
        if (tx->Hpd != AV_HPD_HIGH)
        {
            KfunTxSinkLost(gsv_config,port);
            *port->content.is_PlugTxFsm = AvFsmPlugTxReset;
            KfunPtVarInit(gsv_config,port);
            KfunPtResetPort(gsv_config,port);
            KfunTxSetMuteAv(gsv_config,port);
        ...
        {
            KfunCecUpdateFromEdid(gsv_config,port);
        }
        break;
      ...
      case AvFsmPlugTxHdcp:
        ...
        else if ((hdcp->Authenticated != 0U) || (hdcp->Hdcp2p2TxRunning != 0U))
        {
            *port->content.is_PlugTxFsm = AvFsmPlugTxStable;
            KfunTxClearMuteAv(gsv_config,port);
        }
        ...
        {
            KfunTxHdcpManage(gsv_config,port);
        }
        break;
      case AvFsmPlugTxHpdAntiDither:
        if ((bool)tx->HpdDelayExpire)
        {
            *port->content.is_PlugTxFsm = AvFsmPlugTxEnableTxCore;
            KfunPtEnableTxCore(gsv_config,port);
        }
        else
        {
            if (tx->Hpd != AV_HPD_HIGH)
      ...
      case AvFsmPlugTxStable:
        ...
        {
            KfunTxVideoManage(gsv_config,port);
            KfunTxAudioManage(gsv_config,port);
            KfunTxHdcpManage(gsv_config,port);
        }
        break;
            ...
            if (tx->Hpd != AV_HPD_HIGH)
            {
                if (tx->Hpd == AV_HPD_FORCE_LOW)
                {
                    ...
                    *port->content.is_PlugTxFsm =
                    ...
                    KfunTxUnlockProtection(gsv_config,port);
                }
                else
                {
                    KfunTxSinkLost(gsv_config,port);
...
void AvFsmFunPlugRxFsm(GSV_CONFIG *gsv_config,AvPort *port)
{
    ...
    switch (*port->content.is_PlugRxFsm)
    {
      case AvFsmPlugRxDefault:
        *port->content.is_PlugRxFsm = AvFsmPlugRxReset;
        KfunPrVarInit(port);
        KfunPrResetPort(gsv_config,port);
      ...
      case AvFsmPlugRxInfoUpdate:
        ...
        else
        {
            *port->content.is_PlugRxFsm = AvFsmPlugRxStable;
            rx->IsFreeRun = 1;
 extern AvRet AvUapiRxGetVideoTiming(GSV_CONFIG *gsv_config,pio AvPort *port);
      ...
      case AvFsmPlugRxPullDownHpd:
        ...
        {
            KfunPrepareEdid(gsv_config,port);
        }
        break;
      case AvFsmPlugRxReadTiming:
        ...
        else
        {
            *port->content.is_PlugRxFsm = AvFsmPlugRxInfoUpdate;
            KfunRxReadInfo(gsv_config,port);
        }
        break;
...
void AvFsmFunHdcpFsm(GSV_CONFIG *gsv_config,AvPort *port)
{
    RxVars* rx = port->content.rx;
    ...
    switch (*port->content.is_HdcpFsm)
    {
      case AvFsmHdcp2p2State:
        if ((hdcp->Hdcp2p2Flag == 0) || (rx->IsInputStable == 0))
        {
            *port->content.is_HdcpFsm = AvFsmHdcpDisable;
            ...
            KfunHdcpMuteAv(gsv_config,port);
      ...
      case AvFsmHdcpRepeaterMode:
        ...
        {
            KfunCheckEncryption(gsv_config,port);
        }
        break;
...
void AvFsmFunReceiverFsm(GSV_CONFIG *gsv_config,AvPort *port)
{
    RxVars* rx = port->content.rx;
    bool guard1 = false;
    bool guard2 = false;
        ...
        else
        {
            if (rx->IsInputStable == 1)
            {
                *port->content.is_ReceiverFsm = AvFsmRxReceiving;
                KfunRxClearFlags(gsv_config,port);
            }
        }
        break;
      case AvFsmRxFreerun:
    ...
    if (guard3)
    {
        KfunRxVideoManage(gsv_config,port);
        KfunRxAudioManage(gsv_config,port);
    }
    if (guard2)
    {
        *port->content.is_ReceiverFsm = AvFsmRxFreerun;
    ...
    switch (*port->content.is_CecFsm)
    {
      ...
      case AvFsmCecCmdARC:
        ...
        else
        {
            ...
            {
                AvHandleEvent(gsv_config,port, AvEventCecArcManage, NULL, NULL);
            }
        }
        break;
        ...
        else
        {
            if (tx->PhyAddr != 0)
            {
                *port->content.is_CecFsm = AvFsmCecCmdActiveSource;
                AvHandleEvent(gsv_config,port, AvEventCecSendReportPhysAddress, NULL, NULL);
            }
        }
        break;
...
void AvFsmEnterFunHdcpFsm(GSV_CONFIG *gsv_config,AvPort *port)
{
    *port->content.is_HdcpFsm = AvFsmHdcpDefault;
    KfunHdcpVarInit(gsv_config,port);
}
