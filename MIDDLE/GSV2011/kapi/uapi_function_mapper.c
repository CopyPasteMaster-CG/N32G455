...
{
    AvUapiRxPortInit(gsv_config,port);
    ...
    KfunHdcpReset(gsv_config,port);
    KfunHdcpVarInit(gsv_config,port);
#endif
    return;
}
...
void KfunTxfromRxPacketContent(GSV_CONFIG *gsv_config,AvPort *RxPort, AvPort *TxPort, uint8 *PktContent, uint16 AvBit,
                               PacketType AvPacket, AvPacketType type, AvRxUpdateType update)
{
    if(type == AvVideoPacketType)
    {
        if(((RxPort->content.video->AvailableVideoPackets & AvBit) != 0) || (update == AvRxForce))
        {
            ...
            {
                AvUapiRxGetPacketContent(gsv_config,RxPort, AvPacket, PktContent);
                KfunUpdatePortFromPacket(gsv_config,RxPort, AvPacket, PktContent);
            }
            KfunCheckVspPortContent(gsv_config,TxPort, AvPacket, PktContent);
            KfunUpdatePortFromPacket(gsv_config,TxPort, AvPacket, PktContent);
            AvUapiTxSetPacketContent(gsv_config,TxPort, AvPacket, PktContent, 1);
            TxPort->content.video->AvailableVideoPackets =
                TxPort->content.video->AvailableVideoPackets | AvBit;
        }
        else
        {
            AvUapiTxEnableInfoFrames(gsv_config,TxPort, AvBit, 0);
            TxPort->content.video->AvailableVideoPackets =
                TxPort->content.video->AvailableVideoPackets & (~AvBit);
        }
    }
...
void KfunRxtoTxPacketContent(GSV_CONFIG *gsv_config,AvPort *port, uint8 *PktContent, uint16 AvBit,
                             PacketType AvPacket, AvPacketType type)
{
    ...
    if(type == AvVideoPacketType)
    {
        ...
        {
            AvUapiRxGetPacketContent(gsv_config,port, AvPacket, PktContent);
            KfunUpdatePortFromPacket(gsv_config,port, AvPacket, PktContent);
        }
        while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
        {
            if(CurrentPort->type == HdmiTx)
                KfunTxfromRxPacketContent(gsv_config,port, CurrentPort, PktContent, AvBit, AvPacket, type, AvRxUpdate);
...
void KfunRxVideoManage(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    ...
    ret = AvUapiRxGetVideoPacketStatus(gsv_config,port, &Intpt);
    /* Step 1.1 Clear Interrupt */
    AvUapiRxClearVideoInterrupt(gsv_config,port, &Intpt);

    /* Step 2. Process it if existed */
    if((ret == AvOk) && (Intpt.VsyncLck == 0) && (Intpt.DeRegenLck == 0) && (Intpt.NewTmds == 0) && (Intpt.BadTmdsClk == 0))
    {
        if(Intpt.AvMute)
        {
            AvUapiRxGetAvMute(gsv_config,port);
            PrevPort = NULL;
            CurrentPort = NULL;
            while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
            {
                if(CurrentPort->type == HdmiTx)
                {
                    ...
                    AvUapiOutputDebugMessage(gsv_config,"from Intpt.AvMute-->");
                    AvUapiTxSetAvMute(gsv_config,CurrentPort);
                    CurrentPort->content.video->AvailableVideoPackets =
                        CurrentPort->content.video->AvailableVideoPackets | AV_BIT_GC_PACKET;
                }
                PktContent[0] = 1;
                AvUapiTxSetPacketContent(gsv_config,CurrentPort, AV_PKT_GC_PACKET, PktContent, 1);
                PrevPort = CurrentPort;
                CurrentPort = NULL;
            }
        }
        ...
        {
            AvUapiRxGetHdmiModeSupport(gsv_config,port);
            PrevPort = NULL;
            CurrentPort = NULL;
            while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
            {
                if(CurrentPort->type == HdmiTx)
                {
                    ...
                    else
                    {
                        ...
                        AvUapiTxSetHdmiModeSupport(gsv_config,CurrentPort);
                    }
                }
                PrevPort = CurrentPort;
                CurrentPort = NULL;
        ...
        if(Intpt.Vid3dDet)
        {
            AvUapiRxGetVideoTiming(gsv_config,port);
        }
        if(Intpt.NewTmds)
        {
        }
        ...
        if(Intpt.DeepClrChg)
        {
            AvUapiRxGetHdmiDeepColor(gsv_config,port);
            PrevPort = NULL;
            CurrentPort = NULL;
            while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
            {
                if(CurrentPort->type == HdmiTx)
                {
                    ...
                    AvUapiTxSetHdmiDeepColor(gsv_config,CurrentPort);
                }
                PrevPort = CurrentPort;
                CurrentPort = NULL;
            }
            ...
            while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
            {
                if(CurrentPort->type == HdmiTx)
                {
                    if((port->content.video->AvailableVideoPackets & AV_BIT_GC_PACKET) != 0)
                    {
                        ...
                        AvUapiTxSetHdmiDeepColor(gsv_config,CurrentPort);
                        CurrentPort->content.video->Mute.AvMute = port->content.video->Mute.AvMute;
                        AvUapiOutputDebugMessage(gsv_config,"from Intpt.GcValid-->");
                        AvUapiTxSetAvMute(gsv_config,CurrentPort);
                        CurrentPort->content.video->AvailableVideoPackets =
                            CurrentPort->content.video->AvailableVideoPackets | AV_BIT_GC_PACKET;
                        PktContent[0] = 1;
...
void KfunRxAudioManage(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    ...
    /* Step 1. Get the Packet Status */
    ret = AvUapiRxGetAudioPacketStatus(gsv_config,port, &Intpt);
    /* Step 1.1 Clear Interrupt */
    AvUapiRxClearAudioInterrupt(gsv_config,port, &Intpt);
    ...
    if(ret == AvOk)
    {
        ...
        if(Intpt.CtsThresh)
        {
            AvUapiRxGetHdmiAcrInfo(gsv_config,port);
        }
        if(Intpt.AudFifoOv)
        {
        }
        ...
        {
            KfunRxtoTxPacketContent(gsv_config,port, PktContent, AV_BIT_ACP_PACKET, AV_PKT_ACP_PACKET,
                                    AvAudioPacketType);
        }
        ...
        {
            KfunRxtoTxPacketContent(gsv_config,port, PktContent, AV_BIT_AUDIO_CHANNEL_STATUS,
                                    AV_PKT_AUDIO_CHANNEL_STATUS, AvAudioPacketType);
        }
        if((Intpt.AudIfValid) ||
           ((port->content.rx->ChangedAudioPackets & AV_BIT_AUDIO_INFO_FRAME) != 0))
        {
            KfunRxtoTxPacketContent(gsv_config,port, PktContent, AV_BIT_AUDIO_INFO_FRAME,
        ...
        if(Intpt.NChange)
        {
            ...
            {
                CurrentPort->content.audio->NValue = port->content.audio->NValue;
                AvUapiTxSetAudNValue(gsv_config,CurrentPort);
                PrevPort = CurrentPort;
                CurrentPort = NULL;
            }
        }
    ...
    AvUapiRxAudioManage(gsv_config,port, &Intpt);
...
{
    AvUapiRxClearFlags(gsv_config,port);
    return;
}
...
{
    AvHandleEvent(gsv_config,port, AvEventRxSetHdcpStyle, NULL, NULL);
 ...
 * @note
 */
void KfunHdcpReset(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    port->content.hdcp->SinkNumber = 0;
    port->content.hdcp->SinkTotal = 0;
    port->content.hdcp->BksvPtr = 0;
    port->content.hdcp->AksvInterrupt = 0;
    AvUapiRxSetHdcpMode(gsv_config,port);
    AvUapiHdcp2p2Mode(port);
    AvUapiOutputDebugMessage(gsv_config,"Port %d: HDCP Reset to REC Mode", port->index);
    return;
}
...
void KfunHdcpDetectSource(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    ...
    if(port->type == HdmiRx)
    {
        ...
        if(port->content.hdcp->HdcpNeeded != AV_HDCP_RX_NOT_SUPPORT)
        {
            ...
            {
                /* 1. Get Rx Status */
                ret = AvUapiRxGetHdcpStatus(gsv_config,port, &Intpt);
                ...
                if(ret == AvOk)
                {
                    ...
                    if(Intpt.Encrypted)
                    {
                        if(port->content.rx->VideoEncrypted)
                            AvHandleEvent(gsv_config,port, AvEventPortUpStreamEncrypted, NULL, NULL);
                        else
                            AvHandleEvent(gsv_config,port, AvEventPortUpStreamDecrypted, NULL, NULL);
                    }
                }

                /* 3. Clear Interrupt */
                AvUapiRxClearHdcpInterrupt(gsv_config,port, &Intpt);
...
void KfunHdcpDetectSink(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    ...
    while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
    {
        ...
        {
            /* Get Tx Hdcp Status */
            AvUapiTxGetHdcpStatus(gsv_config,CurrentPort);
            /* Support HDCP */
    ...
    while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
    {
        ...
        if(CurrentPort->type == HdmiTx)
        {
            ...
            if(port->type == HdmiRx)
            {
                ...
                {
                    AvUapiTxEncryptSink(gsv_config,CurrentPort);
                }
            }
            ...
            if(port->content.rx->VideoEncrypted == 0)
            {
                if(CurrentPort->content.tx->HdmiMode == 0)
                {
                    if(CurrentPort->content.video->Mute.BlkMute == 1)
                    {
                        ...
                        AvUapiTxSetBlackMute(gsv_config,CurrentPort);
                    }
                }
                else if(CurrentPort->content.hdcptx->HdcpEnabled == 1)
                {
                    AvUapiTxDecryptSink(gsv_config,CurrentPort);
                }
            }
        }
...
void KfunTxUnlockProtection(GSV_CONFIG *gsv_config,pio AvPort *port)
{
    ...
    AvUapiTxDisableCore(gsv_config,port);
    AvUapiTxEnableCore(gsv_config,port);
...
void KfunTxSetMuteAv(GSV_CONFIG *gsv_config,pio AvPort *port)
{
    ...
    if(port->content.tx->HdmiMode)
    {
        ...
        AvUapiOutputDebugMessage(gsv_config,"from Kfun TX SetMuteAv-->");
#endif
        AvUapiTxSetAvMute(gsv_config,port);
        /* BlackMute */
...
#if AvEnableDebugHdcp
        AvUapiOutputDebugMessage(gsv_config,"from Kfun TX ClearMuteAv-->");
#endif
        AvUapiTxSetAvMute(gsv_config,port);
        /* BlackMute */
...
void KfunHdcpMuteAv(GSV_CONFIG *gsv_config,pio AvPort *port)
{
    ...
    while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
    {
        ...
        if(CurrentPort->type == HdmiTx)
        {
            ...
            {
                KfunTxSetMuteAv(gsv_config,CurrentPort);
            }
        }
        PrevPort = CurrentPort;
    }
...
void KfunTxHdcpManage(GSV_CONFIG *gsv_config,pio AvPort *port)
{
    if(port->content.hdcptx->HdcpEnabled)
    {
        ...
        {
            AvUapiTxClearBksvReady(gsv_config,port);
        }
...
{
    AvUapiTxMuteTmds(gsv_config,port, true);
    return;
}
...
{
    AvUapiTxMuteTmds(gsv_config,port, false);
    return;
}
...
void KfunTxConnection(GSV_CONFIG *gsv_config,pio AvPort *port)
{
    if(port)
    {
        ...
        if(port->type == HdmiRx)
        {
            ...
            while(TxPort)
            {
                AvUapiTxGetHdcpStatus(gsv_config,TxPort);
...
{
    AvHandleEvent(gsv_config,port, AvEventPortDownStreamDisconnected, NULL, NULL);
    return AvOk;
}
...
{
    AvUapiResetPort(gsv_config,port);
    return;
}
...
{
    AvUapiRxSetHpdDown(gsv_config,port);
    port->content.rx->Hpd = AV_HPD_LOW;
    port->content.rx->HpdDelayExpire = 0;
    port->content.hdcp->HdcpError = 0;
    return;
...
{
    AvUapiRxSetHpdUp(gsv_config,port);
...
void KfunPrepareEdid(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    if(port->content.rx->EdidStatus != AV_EDID_UPDATED)
        AvHandleEvent(gsv_config,port, AvEventRxPrepareEdid, NULL, NULL);
}
    ...
    while(KfunFindVideoNextNode(port, &PrevPort, &CurrentPort) == AvOk)
    {
        if(CurrentPort->type == HdmiTx)
        {
            AvMemcpy(&(CurrentPort->content.video->timing),
                     ...
                     sizeof(VideoTiming));
            KfunTxSetColorDepth(port, CurrentPort);
            AvUapiTxSetVideoTiming(gsv_config,CurrentPort);
        }
        PrevPort = CurrentPort;
    }
    return;
...
void KfunPtVarInit(GSV_CONFIG *gsv_config,pout AvPort *port)
{
    ...
    port->content.video->AvailableVideoPackets = 0;
    port->content.audio->AvailableAudioPackets = 0;
    AvHandleEvent(gsv_config,port, AvEventTxSetHdcpStyle, NULL, NULL);
    return;
}
...
AvRet KfunTxVideoManage(GSV_CONFIG *gsv_config,pout AvPort *port)
{
    ...
    if(KfunFindVideoRxFront(port, &SourcePort) == AvOk)
    {
        ...
        if(port->content.tx->InfoReady <= TxHdcpManageThreshold)
        {
            ...
            //AvUapiOutputDebugMessage(gsv_config,"Port%d: InfoReady = %d", port->index, port->content.tx->InfoReady);
        }
        ...
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_AV_INFO_FRAME,
        ...
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_SPD_PACKET,
                                      AV_PKT_SPD_PACKET, AvVideoPacketType, AvRxUpdate);
        }
        if((DifferentVideoPackets & AV_BIT_HDR_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_HDR_PACKET,
                                      AV_PKT_HDR_PACKET, AvVideoPacketType, AvRxUpdate);
        }
        if((DifferentVideoPackets & AV_BIT_MPEG_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_MPEG_PACKET,
                                      AV_PKT_MPEG_PACKET, AvVideoPacketType, AvRxUpdate);
        }
        if((DifferentVideoPackets & AV_BIT_VS_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_VS_PACKET,
        ...
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_ISRC1_PACKET,
                                      AV_PKT_ISRC1_PACKET, AvVideoPacketType, AvRxUpdate);
        }
        if((DifferentVideoPackets & AV_BIT_ISRC2_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_ISRC2_PACKET,
                                      AV_PKT_ISRC2_PACKET, AvVideoPacketType, AvRxUpdate);
        }
        if((DifferentVideoPackets & AV_BIT_GMD_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_GMD_PACKET,
        ...
        if((DifferentVideoPackets & AV_BIT_GC_PACKET) != 0)
        {
            ...
            {
                AvUapiRxGetHdmiDeepColor(gsv_config,SourcePort);
                KfunTxSetColorDepth(SourcePort, port);
                AvUapiTxSetHdmiDeepColor(gsv_config,port);
                port->content.video->AvailableVideoPackets =
                    port->content.video->AvailableVideoPackets | AV_BIT_GC_PACKET;
                PktContent[0] = 1;
                AvUapiTxSetPacketContent(gsv_config,port, AV_PKT_GC_PACKET, PktContent, 1);
            }
            else
            {
                port->content.video->AvailableVideoPackets =
        ...
        /* Call Silicon Layer */
        AvUapiTxVideoManage(gsv_config,port);
    }
    else
    {
        ret = AvNotAvailable;
...
AvRet KfunTxAudioManage(GSV_CONFIG *gsv_config,pout AvPort *port)
{
    ...
    if(KfunFindAudioRxFront(port, &SourcePort) == AvOk)
    {
        ...
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_AUDIO_CHANNEL_STATUS,
                                      AV_PKT_AUDIO_CHANNEL_STATUS, AvAudioPacketType, AvRxUpdate);
        }
        if((DifferentAudioPackets & AV_BIT_AUDIO_INFO_FRAME) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_AUDIO_INFO_FRAME,
                                      AV_PKT_AUDIO_INFO_FRAME, AvAudioPacketType, AvRxUpdate);
        }
        if((DifferentAudioPackets & AV_BIT_ACP_PACKET) != 0)
        {
            KfunTxfromRxPacketContent(gsv_config,SourcePort, port, PktContent, AV_BIT_ACP_PACKET,
        ...
        /* Call Silicon Layer */
        AvUapiTxAudioManage(gsv_config,port);
           ...
           (SourcePort->type == VideoColor)))
    {
        switch(Pkt)
        {
            case AV_PKT_AV_INFO_FRAME:
            {
                ...
                {
                    case VideoScaler:
                        KfunCheckVideoScaler(gsv_config,SourcePort);
                        if(VicUpdateFlag == 0)
                        {
                        ...
                        break;
                    case VideoColor:
                        KfunCheckVideoColor(gsv_config,SourcePort);
                        if(VicUpdateFlag == 0)
                        {
    ...
    switch (Pkt)
    {
        ...
        case AV_PKT_AV_INFO_FRAME:
            ...
            if(Value != 0)
            {
                ...
                AvUapiOutputDebugMessage(gsv_config,"Port%d: Get Vic = [%x]", port->index, Value);
            }
            /* Color Space */
                    ...
                    if(port->content.audio->SampFreq != (AvAudioSampleFreq)ChannelStatusSfTable[i])
                    {
                        ...
                        switch(port->content.audio->SampFreq)
                        {
                            case AV_AUD_FS_32KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 32KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_44KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 44KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_48KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 48KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_88KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 88KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_96KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 96KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_176KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 176KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_192KHZ:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: 192KHz Detected", port->index);
                                break;
                            case AV_AUD_FS_HBR:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: HBR Detected", port->index);
                                break;
                            case AV_AUD_FS_FROM_STRM:
                                AvUapiOutputDebugMessage(gsv_config,"Port%d: Audio From Stream Detected", port->index);
                                break;
                        }
                    }
                    break;
...
AvRet ValidMsgSize (CecMessage Opcode, uint8 MsgSize)
{
    ...
    {
        AvUapiOutputDebugMessage(gsv_config,"CEC: Msg size = %d", MsgSize);
        if(ret == AvInvalidParameter)
        {
            AvUapiOutputDebugMessage(gsv_config,"CEC: Invalid msg size is %d,Opcode 0x%x.", MsgSize, Opcode);
            AvUapiOutputDebugMessage(gsv_config,"CEC: take no action");
        }
    }
    return ret;
}
 ...
 * @note
 */
AvRet KfunCecRxMsgProcess(GSV_CONFIG *gsv_config,AvPort *port)
{
    AvRet ret = AvNotAvailable;
    ...
    AvUapiOutputDebugMessage(gsv_config,"CEC: Message received. Opcode=0x%x Src=%d Dst=%d Data=%x",
    ...
    {
        /*
        AvUapiOutputDebugMessage(gsv_config,"CEC: Message received. Opcode=0x%x Src=%d Dst=%d\n\r",
        ...
        */
        ...
        switch (AV_CEC_OPCODE(Msg))
        {
            case AV_CEC_MSG_ROUTE_CHANGE:
                AvHandleEvent(gsv_config,port, AvEventCecMsgRouteChange, Msg, NULL);
                break;

            case AV_CEC_MSG_ROUTE_INFO:
                AvHandleEvent(gsv_config,port, AvEventCecMsgRouteInfo, Msg, NULL);
                break;

            case AV_CEC_MSG_ACTIVE_SRC:
                AvHandleEvent(gsv_config,port, AvEventCecMsgActiveSource, Msg, NULL);
                break;


            case AV_CEC_MSG_GIVE_PHYS_ADDR:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGivePhyAddr, Msg, NULL);
                break;

            case AV_CEC_MSG_SET_STRM_PATH:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSetStreamPath, Msg, NULL);
                break;

            case AV_CEC_MSG_STANDBY:
                AvHandleEvent(gsv_config,port, AvEventCecMsgStandby, Msg, NULL);
                break;

            case AV_CEC_MSG_ABORT:
                AvHandleEvent(gsv_config,port, AvEventCecMsgAbort, Msg, NULL);
                break;

            case AV_CEC_MSG_FEATURE_ABORT:
                AvHandleEvent(gsv_config,port, AvEventCecMsgFeatureAbort, Msg, NULL);
                break;

            case AV_CEC_MSG_GIVE_PWR_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGivePowerStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_PWR_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportPowerStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_GIVE_OSD_NAME:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGiveOsdName, Msg, NULL);
                break;

            case AV_CEC_MSG_GET_VENDOR_ID:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGetVendorId, Msg, NULL);
                break;

            case AV_CEC_MSG_GET_CEC_VERSION:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGetCecVersion, Msg, NULL);
                break;

            case AV_CEC_MSG_REQUEST_ARC_INITIATION:
                AvKapiArcEnable(port, 1);
                /* port->content.cec->EnableARC = AV_CEC_ARC_TO_INITIATE; */
                AvHandleEvent(gsv_config,port, AvEventCecMsgRequestArcInitiation, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_ARC_INITIATED:
                AvKapiArcEnable(port, 1);
                port->content.cec->EnableARC = AV_CEC_ARC_INITIATED;
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportArcInitiated, Msg, NULL);
                break;

            case AV_CEC_MSG_REQUEST_ARC_TERMINATION:
                AvKapiArcEnable(port, 0);
                /* port->content.cec->EnableARC = AV_CEC_ARC_TO_TERMINATE; */
                AvHandleEvent(gsv_config,port, AvEventCecMsgRequestArcTermination, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_ARC_TERMINATED:
                AvKapiArcEnable(port, 0);
                port->content.cec->EnableARC = AV_CEC_ARC_TERMINATED;
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportArcTerminated, Msg, NULL);
                break;

            case AV_CEC_MSG_USER_CONTROL_PRESSED:
                AvHandleEvent(gsv_config,port, AvEventCecMsgUserControlPressed, Msg, NULL);
                break;

            case AV_CEC_MSG_USER_CONTROL_RELEASED:
                AvHandleEvent(gsv_config,port, AvEventCecMsgUserControlReleased, Msg, NULL);
                break;
            case AV_CEC_MSG_IMAGE_VIEW_ON:
                AvHandleEvent(gsv_config,port, AvEventCecMsgImageViewOn, Msg, NULL);
                break;

            case AV_CEC_MSG_TEXT_VIEW_ON:
                AvHandleEvent(gsv_config,port, AvEventCecMsgTextViewOn, Msg, NULL);
                break;

            case AV_CEC_MSG_SYSTEM_AUDIO_MODE_REQUEST:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSystemAudioModeRequest, Msg, NULL);
                break;

            case AV_CEC_MSG_DECK_CONTROL:
                AvHandleEvent(gsv_config,port, AvEventCecMsgDeckControl, Msg, NULL);
                break;

            case AV_CEC_MSG_PLAY:
                AvHandleEvent(gsv_config,port, AvEventCecMsgPlay, Msg, NULL);
                break;

            case AV_CEC_MSG_GIVE_AUDIO_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGiveAudioStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_GIVE_SYSTEM_AUDIO_MODE_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgGiveSystemAudioModeStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_REQUEST_SHORT_AUDIO_DESCRIPTOR:
                AvHandleEvent(gsv_config,port, AvEventCecMsgRequestShortAudioDescriptor, Msg, NULL);
                break;

            case AV_CEC_MSG_SYSTEM_AUDIO_MODE_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSystemAudioModeStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_SET_SYSTEM_AUDIO_MODE:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSetSystemAudioMode, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_AUDIO_STATUS:
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportAudioStatus, Msg, NULL);
                break;

            case AV_CEC_MSG_SET_AUDIO_RATE:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSetAudioRate, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_SHORT_AUDIO_DESCRIPTOR:
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportShortAudioDescriptor, Msg, NULL);
                break;

            case AV_CEC_MSG_REPORT_PHYS_ADDR:
                AvHandleEvent(gsv_config,port, AvEventCecMsgReportPhyAddr, Msg, NULL);
                break;

            case AV_CEC_MSG_MENU_REQUEST:
                AvHandleEvent(gsv_config,port, AvEventCecMsgMenuRequest, Msg, NULL);
                break;

            case AV_CEC_MSG_REQ_ACTIVE_SRC:
                AvHandleEvent(gsv_config,port, AvEventCecMsgRequestActiveSource, Msg, NULL);
                break;

            case AV_CEC_MSG_SET_MENU_LANGUAGE:
                AvHandleEvent(gsv_config,port, AvEventCecMsgSetMenuLanguage, Msg, NULL);
                break;

            case AV_CEC_MSG_DEVICE_VENDOR_ID:
                AvHandleEvent(gsv_config,port, AvEventCecMsgDeviceVendorId, Msg, NULL);
                break;
            ...
            default:
                ...
                {
                    AvHandleEvent(gsv_config,port, AvEventCecSendFeatureAbort, Msg, NULL);
    ...
    else
    {
        switch (AV_CEC_OPCODE(Msg))
        {
            ...
            if ( (!AV_CEC_BROADCAST(Msg)) && (port->content.cec->RxLen > 2 ))
                {
                    AvHandleEvent(gsv_config,port, AvEventCecSendFeatureAbortReason, Msg, NULL);
 ...
 * @note
 */
AvRet KfunPrepareLogAddr (GSV_CONFIG *gsv_config,AvPort* port)
{
    uint8 ErrCode = 0;
    uint8 FoundLogAddr = 0;
    /* AvUapiOutputDebugMessage(gsv_config,"CEC: TxSendFlag = %d.", port->content.cec->TxSendFlag); */

    if(port->content.cec->TxSendFlag == AV_CEC_TX_TO_SEND)
    {
        AvUapiOutputDebugMessage(gsv_config,"CEC: Found TxSendFlag = 1!!!!!!!!");
        return AvOk;
    }
            ...
            (port->content.cec->TxSendFlag == AV_CEC_TX_SEND_FAIL))
    {
            ...
            port->content.cec->Timeout)
        {
            ...
            if (port->content.cec->TxReady)
            {
                ...
                AvUapiOutputDebugMessage(gsv_config,"CEC Port %d: %d Exists",
                ...
                if(port->content.cec->AddrIndex == 5)
                {
                    ...
                    AvHandleEvent(gsv_config,port, AvEventCecLogAddrAlloc, &FoundLogAddr, NULL);
                }
#endif
            ...
            else if(port->content.cec->Timeout)
            {
                ...
                if (port->content.cec->NackCount == (3 + 1)) /* CEC_RETRY_COUNT = 3 */
                {
                    ...
                    FoundLogAddr = port->content.cec->AddrIndex;
                    AvHandleEvent(gsv_config,port, AvEventCecLogAddrAlloc, &FoundLogAddr, NULL);
                    AvUapiOutputDebugMessage(gsv_config,"CEC Port %d: %d Available",
                                             port->index, port->content.cec->AddrIndex);
                    if(port->content.cec->AddrIndex < 15)
                        port->content.cec->AddrIndex++;
                }
...
AvRet KfunCecManage(AvPort *port)
{
...
#if AvCecLinkageMode
                AvHandleEvent(gsv_config,port, AvEventCecSendRoutingInformation, NewSpa, NULL);
#endif
            }
            /* 2.2.2 new source */
...
#if AvCecLinkageMode
                AvHandleEvent(gsv_config,port, AvEventCecSendRoutingChange, OldSpa, NewSpa);
#endif
    ...
    else
    {
        ...
        {
#if AvCecLinkageMode
            AvHandleEvent(gsv_config,port, AvEventCecSendInactiveSource, NULL, NULL);
#endif
...
AvRet KfunCecUpdateFromEdid(GSV_CONFIG *gsv_config,AvPort *port)
{
    ...
    if(port->type == HdmiTx)
    {
        AvUapiTxReadEdid(gsv_config,port, Edid, AvEdidMaxSize);
        /* Incorrect Edid, Return */
        if((Edid[6] != 0xFF) || (Edid[7] != 0x00))
        {
            AvUapiOutputDebugFsm(gsv_config,"Port %d: Edid Checksum Fail, Abort",port->index);
#if AvForceDefaultEdid
            port->content.tx->IgnoreEdidError = 1;
            KfunTxUseDefaultEdid(gsv_config,port);
#else
            port->content.tx->Hpd = AV_HPD_FORCE_LOW;
#endif
            return AvError;
        }
        SpaLocation = KfunFindCecSPAFromEdid(gsv_config,Edid, Spa);
           ...
           (Edid[255] != port->content.tx->EdidCks[1]))
        {
            ...
            AvHandleEvent(gsv_config,port, AvEventPortEdidReady, Edid, Spa);
        }
        else
        {
            if(KfunFindVideoRxFront(port, &SourcePort) == AvOk)
...
AvRet KfunTxUseDefaultEdid(GSV_CONFIG *gsv_config,pin AvPort *port)
{
    ...
    AvHandleEvent(gsv_config,port, AvEventTxDefaultEdid, NULL, NULL);
    ...
    while(offset < DTDstart)
    {
        ...
        if((currptr[offset]&0xe0) == 0x60)
        {
            ...
            else
            {
                ...
                AvUapiOutputDebugMessage(gsv_config,"SPA Found =%02x,%02x", spa[0], spa[1]);
                return (offset+4);
            }
        }
...
void KfunTxSetColorDepth(AvPort *RxPort, AvPort *TxPort)
{
    ...
    //AvUapiOutputDebugMessage(gsv_config,"Tx Port %d: Color Depth = %d", TxPort->index, TxPort->content.video->Cd);
}
