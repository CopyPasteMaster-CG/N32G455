...
const uint16 AvEdidSupportFreq  =
                     AvEdidBitFreq6G    |
                     AvEdidBitFreq4P5G  |
                     AvEdidBitFreq3P75G |
                     AvEdidBitFreq3G    |
...
static const uint16 AvEdidSupportAudioFormat  =
                           ...
                           AvEdidBitAudioFormatMAT          |
                           AvEdidBitAudioFormatDTSHD;
//static const uint16 AvEdidSupportAudioFormat  = 0x3FFF; //  所有GSV支持的音频格式
...
const uint16 AvEdidVesaParamForce =  0;
const uint8 AvEdidCeaParamForce  =  0;//AvEdidBitCeaSVD | AvEdidBitCeaVSDBHF;
const uint16 AvEdidCeabParamForce = 0;//AvEdidBitCeabCDB | AvEdidBitCeabVCDB;
const uint16 AvEdidVesaParamRemove = 0;
const uint8 AvEdidCeaParamRemove  = 0;
const uint16 AvEdidCeabParamRemove = 0;
...
//static const uint8 AvEdidIDManufacturerName[] =  {"    "};
//static const uint8 AvEdidProductName[] =  {"isw.com"};
...
//        }
    ...
    void AvEdidFunFullAnalysis(GSV_CONFIG *gsv_config,AvEdidReg *EdidReg,uint8 *InEdid)
    {
        uint16  iCeabParam = 0;
        uint8   i = 0;
        uint8   j = 0;
    ...
    void AvEdidFuncCeaProcess(GSV_CONFIG *gsv_config,AvEdidReg *EdidReg,uint8 *InEdid,uint8 *SinkEdid,uint8 *OutEdid)
    {
        uint16  iCeabParam = 0;
        uint16  sCeabParam = 0;
        uint8   i = 0;
        ...
        {
            oLen       = AvEdidFuncCeabSVD(gsv_config,EdidReg, InEdid, iCeabParam, SinkEdid, sCeabParam, OutEdid, oOffset);
        }
        ...
        {
            oLen       = AvEdidFuncCeabVsdbHdmi(gsv_config,EdidReg, InEdid, iCeabParam, SinkEdid, sCeabParam, OutEdid, oOffset);
        }
    ...
    uint8 AvEdidFuncCeabSVD(GSV_CONFIG *gsv_config,AvEdidReg *EdidReg,uint8 *InEdid,uint16 InCeabParam,uint8 *SinkEdid,uint16 SinkCeabParam,uint8 *OutEdid,uint8 OutEdidOffset)
    {
...
//        EdidReg->EdidCeaAudioByte3[9]     = 0x01;
//        EdidReg->EdidCeaAudioByte3[10]    = 0x00;
//        EdidReg->EdidCeaAudioByte3[11]    = 0x01;
...
//        ///////////////////////////////////////     原始程序屏蔽结束-1
        ...
        //  环出的HDMI 读取到的EDID 带有音频格式，则输出的EDID模拟环出的音频格式
        
        SelectValue = 1;
        if(InEdidOffset != 0)
        {
            SelectValue = SelectValue + 1;
            EdidReg->EdidCeaAudioCheck[0]     = 0;
            EdidReg->EdidCeaAudioCheck[1]     = 0;
            EdidReg->EdidCeaAudioCheck[2]     = 0;
            EdidReg->EdidCeaAudioCheck[3]     = 0;
            EdidReg->EdidCeaAudioCheck[4]     = 0;
            EdidReg->EdidCeaAudioCheck[5]     = 0;
            EdidReg->EdidCeaAudioCheck[6]     = 0;
            EdidReg->EdidCeaAudioCheck[7]     = 0;
            EdidReg->EdidCeaAudioCheck[8]     = 0;
            EdidReg->EdidCeaAudioCheck[9]     = 0;
            EdidReg->EdidCeaAudioCheck[10]    = 0;
            EdidReg->EdidCeaAudioCheck[11]    = 0;
            EdidReg->EdidCeaAudioCheck[12]    = 0;
            EdidReg->EdidCeaAudioCheck[13]    = 0;
            AvEdidFuncCeabAudioRead(EdidReg, InEdid, InEdidLen, InEdidOffset);
            for(bLen=0;bLen<InEdidLen;bLen++)
            {
                OutEdid[OutEdidOffset+bLen+1] = InEdid[InEdidOffset+bLen+1];
            }
            if(EdidReg->EdidCeaAudioCheck[0]){
                EdidReg->EdidCeaAudioCheck[0]++;
            }
        }else{
            //  否则，按照默认的方式处理音频格式
            if(AvEdidSupportAudioFormat & AvEdidBitAudioFormatLPCM)
...
//        ///////////////////////////////////////     原始程序屏蔽结束-2
        ...
        if(InEdidOffset != 0){
            ...
            {
                OutEdid[OutEdidOffset+bLen+1] = 0x83;
                OutEdid[OutEdidOffset+bLen+2] = InEdid[InEdidOffset+bLen+2];
                OutEdid[OutEdidOffset+bLen+3] = 0x00;
                OutEdid[OutEdidOffset+bLen+4] = 0x00;
                bLen = bLen+4;
            }
    ...
    uint8 AvEdidFuncCeabVsdbHdmi(GSV_CONFIG *gsv_config,AvEdidReg *EdidReg,uint8 *InEdid,uint16 InCeabParam,uint8 *SinkEdid,uint16 SinkCeabParam,uint8 *OutEdid,uint8 OutEdidOffset)
    {
        uint8   InEdidOffset   = (InCeabParam>>8)&0xff;
        uint8   SinkEdidOffset = (SinkCeabParam>>8)&0xff;
        uint8   SelectValue = 0;
