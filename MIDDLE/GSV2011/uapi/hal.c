...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvHookI2cRead(devAddress, regAddress, avdata, count);
    return ret;
}
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvHookI2cWrite(devAddress, regAddress, avdata, count);
    return ret;
}
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvUartTxByte(avdata, avsize);
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvUartRxByte(avdata);
    return ret;
}
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvGetMilliSecond(ms);
    return ret;
}
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvGetKey(avdata);
    return ret;
}
...
AvRet AvHalGetElapsedMilliSecond(GSV_CONFIG *gsv_config,pin uint32 *oldTime, pout uint32 *elapsedTime)
{
    AvRet ret = AvOk;
    uint32 currTime = 0;
    gsv_config->AvGetMilliSecond(&currTime);
...
AvRet AvHalI2cRdMultiField(GSV_CONFIG *gsv_config,pin uint32 devAddress, pin uint32 regAddress, pin uint16 number, pout uint8 *avdata)
{
    ...
    ret = gsv_config->AvHookI2cRead(devAddress, regAddress, avdata, number);
...
AvRet AvHalI2cWrMultiField(GSV_CONFIG *gsv_config,pin uint32 devAddress, pin uint32 regAddress, pin uint16 number, pin uint8 *avdata)
{
    ...
    ret = gsv_config->AvHookI2cWrite(devAddress, regAddress, avdata, number);
...
{
    AvRet ret = AvOk;
    ret = gsv_config->AvHookI2cRead(devAddress, regAddress, avdata, 1);
...
AvRet AvHalI2cWriteField8(GSV_CONFIG *gsv_config,pin uint32 devAddress, pin uint32 regAddress, pin uint8 mask, pin uint8 bitPos, pin uint8 fieldVal)
{
    ...
    if(mask != 0xff)
    {
        gsv_config->AvHookI2cRead(devAddress, regAddress, &val, 1);
        val = (val & ~mask) | ((fieldVal << bitPos) & mask);
    }
    ret = gsv_config->AvHookI2cWrite(devAddress, regAddress, &val, 1);
    return ret;
}
...
AvRet AvHalI2cReadField32(GSV_CONFIG *gsv_config,pin uint32 devAddress, pin uint32 regAddress, pin uint8 msbMask, pin uint8 lsbMask, pin uint8 lsbPos, pin uint8 endian, pin uint8 fldSpan, pout uint32 *avdata)
{
    ...
    *avdata = 0;
    ...
    ret = gsv_config->AvHookI2cRead(devAddress, regAddress, bytes, fldSpan);
...
AvRet AvHalI2cWriteField32 (GSV_CONFIG *gsv_config,pin uint32 devAddress, pin uint32 regAddress, pin uint8 msbMask, pin uint8 lsbMask, pin uint8 lsbPos, pin uint8 endian, pin uint8 fldSpan, pin uint32 avdata)
{
    ...
    if (endian == AvBigEndian)
    {
        ...
        ret = gsv_config->AvHookI2cWrite(devAddress, regAddress, bytes, fldSpan);
    }
    else
    {
 ...
 * @note   none
 */
AvRet AvHalGetIrda(GSV_CONFIG *gsv_config,uint8 *avdata)
{
    AvRet ret = AvOk;
    ret = gsv_config->AvIrdaRxByte(avdata);
    return ret;
}
