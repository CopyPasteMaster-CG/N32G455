...
//uint8  EdidHdmi2p0     = 1;
//uint8  LogicOutputSel  =0;  ///  0 dec  display     1  === /enc  capture
...
//extern AvEdidReg gsv_config->DevEdidReg;
const extern uint16    AvEdidSupportFreq;
const extern uint8     AvEdidVesaParamForce;
const extern uint8     AvEdidCeaParamForce;
const extern uint16    AvEdidCeabParamForce;
const extern uint8     AvEdidVesaParamRemove;
const extern uint8     AvEdidCeaParamRemove;
const extern uint16    AvEdidCeabParamRemove;
//uint8 InEdid[256];
//uint8 SinkEdid[256];
//uint8 OutEdid[256];
AvRet AvEdidPortManage(GSV_CONFIG *gsv_config,AvPort *RxPort);
AvRet AvEdidPortAnalysis(GSV_CONFIG *gsv_config,AvPort *port);
 ...
 * @return none
 */
AvRet AvHandleEvent(GSV_CONFIG *gsv_config,AvPort *port, AvEvent event, uint8 *wparam, uint8 *pparam)
{
    AvRet ret = AvOk;
    AvPort *TempPort;
#if AvEnableCecFeature /* CEC Related */
    ...
    switch(event)
    {
        ...
        case AvEventPortDownStreamSending:
            if(KfunFindVideoRxFront(port, &TempPort) == AvOk)
            {
                if(TempPort->type == HdmiRx)
                {
                    ...
                    KfunTxSetHdmiModeSupport(gsv_config,port);
                }
            }
            break;
        ...
        case AvEventCecTxTimeout:
            AvHandleEvent(gsv_config,port, AvEventCecTxError, 0, NULL);
            break;
        ...
        case AvEventCecLogAddrAlloc:
            ...
            if(*wparam == AvCecLogicAddress)
            {
                ...
                AvUapiOutputDebugMessage(gsv_config,"CEC Port %d: Set Logical Address = %d",
            ...
            if(port->content.cec->TxSendFlag == AV_CEC_TX_SEND_SUCCESS)
            {
                if(port->content.cec->ARCTryCount < 5)
                {
                    ...
                    if(port->content.cec->EnableARC == AV_CEC_ARC_TO_INITIATE)
                        AvHandleEvent(gsv_config,port, AvEventCecSendInitiateARC, NULL, NULL);
                    else if(port->content.cec->EnableARC == AV_CEC_ARC_TO_TERMINATE)
                        AvHandleEvent(gsv_config,port, AvEventCecSendTerminateARC, NULL, NULL);
                }
            }
            break;
#endif
        ...
        case AvEventCecSendRoutingChange:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending routing change. From Inp=%04x to Inp=%04x",
        ...
        case AvEventCecSendRoutingInformation:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending routing information. Inp=%04x", ((Spa[0]<<8)+Spa[1]));
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendReportPhysAddress:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Report phys address=%x", port->content.tx->PhyAddr);
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendActiveSource:
            ...
            AV_CEC_SET_PHYS_ADDR1(TxContent, port->content.tx->PhyAddr);
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending active source=%04x", port->content.tx->PhyAddr);
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendInactiveSource:
            ...
            AV_CEC_SET_PHYS_ADDR1(TxContent, port->content.tx->PhyAddr);
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Inactive source=%04x", port->content.tx->PhyAddr);
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendFeatureAbort:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending feature abort");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendPowerStatus:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending report power status");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendSetOsdName:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Set OSD name");
            AvKapiCecSendMessage(port);
            break;
        case AvEventCecSendDeviceVendorID:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Vendor ID");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendCecVersion:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending CEC Version 1.4");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendFeatureAbortReason:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending feature abort. Abort Reason=%02x", *wparam);
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendReqActiveDevice:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC MSG: Sending active source request");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendReportAudioStatus:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending audio status");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendReportSystemAudioModeStatus:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending audio mode status");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendReportShortAudioDecriptor:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending audio capability");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecSendInitiateARC:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Initiate ARC");
            AvKapiCecSendMessage(port);
            break;
        case AvEventCecSendTerminateARC:
            ...
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Terminate ARC");
            AvKapiCecSendMessage(port);
            break;
        case AvEventCecSendMenuStatus:
            ...
            if(RxContent[2] == 0x00) /* query = deactivate */
                ...
                TxContent[2] = 0x01;
            AvUapiOutputDebugMessage(gsv_config,"CEC: Sending Menu Status");
            AvKapiCecSendMessage(port);
            break;
        ...
        case AvEventCecMsgRouteChange:
                ...
                /* New address is me */
            {
                Spa[0] = port->content.tx->PhyAddr>>8;
                Spa[1] = port->content.tx->PhyAddr & 0xff;
                AvHandleEvent(gsv_config,port, AvEventCecSendRoutingInformation, Spa, NULL);
                AvUapiOutputDebugMessage(gsv_config,"CEC: Routing change. Old PA=%04x  New PA=%04x",
                                         AV_CEC_PHYS_ADDR1(RxContent),
                                         AV_CEC_PHYS_ADDR2(RxContent));
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
            }
            break;
        ...
        case AvEventCecMsgRouteInfo:
            ...
            if (AV_CEC_BROADCAST(RxContent))
            {
                ...
                Spa[1] = port->content.tx->PhyAddr & 0xff;
                AvHandleEvent(gsv_config,port, AvEventCecSendRoutingInformation, Spa, NULL);
                AvUapiOutputDebugMessage(gsv_config,"CEC: Routing Info. Active Route=%04x",
                                         AV_CEC_PHYS_ADDR1(RxContent));
            }
            else
            {
        ...
        case AvEventCecMsgActiveSource:
            ...
            if (AV_CEC_BROADCAST(RxContent))
            {
                ...
                if((InpIndex <= ((port->content.cec->InputCount)&0xf)) &&
                   (InpIndex != 0))
                {
                    AvUapiOutputDebugMessage(gsv_config,"************Active Source Request From %04x (Port %d)",
                                             Recv_Phy_Addr, InpIndex);
#if AvCecLinkageMode
                    /* Active Input needs to be switched */
                    if(InpIndex != (port->content.cec->InputCount>>4))
                    {
                        AvUapiOutputDebugMessage(gsv_config,"CEC Source try to connect Rx%d - Tx%d", InpIndex, port->index-3);
                        ...
                        AvApiConnectPort(gsv_config,&TempPort[InpIndex-1], &TempPort[2], AvConnectAV);
                    }
                    /* Current Active Input is already streaming  */
                    else
                    {
                        AvUapiOutputDebugMessage(gsv_config,"Current Input %d is already streaming", InpIndex);
                    }
#endif
                }
                else
        ...
        case AvEventCecMsgGivePhyAddr:
            ...
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Give physical address");
                AvHandleEvent(gsv_config,port, AvEventCecSendReportPhysAddress, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgSetStreamPath:
            ...
            if (AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"************Set Stream Path Request to Source=%04x", AV_CEC_PHYS_ADDR1(RxContent));
                if (AV_CEC_PHYS_ADDR1(RxContent)==port->content.tx->PhyAddr)
                {
                    AvHandleEvent(gsv_config,port, AvEventCecSendActiveSource, 0, NULL);
                }
                else
                {
#if AvCecLinkageMode
                    ...
                    if (InpIndex <= port->content.cec->InputCount)
                    {
                        ...
                        {
                            /* Find Device's first port (Rx Port) */
                            AvUapiOutputDebugMessage(gsv_config,"CEC TV try to connect Rx%d - Tx%d", InpIndex, port->index-3);
                            TempPort = (AvPort*)port->device->port;
                            port->content.cec->InputCount = (InpIndex<<4) | (port->content.cec->InputCount & 0xf);
                            AvApiConnectPort(gsv_config,&TempPort[InpIndex-1], &TempPort[2], AvConnectAV);
                        }
        ...
        case AvEventCecMsgStandby:
            ...
            ret = AvHandleEvent(gsv_config,port, AvEventCecRxMessageRespond,
                                    Msg_Len,
                                    NULL);
            if ( ret == AvOk )
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: To be in Standby mode");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: is already in Standby mode, ignore");
            }
            break;
        ...
        case AvEventCecMsgAbort:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Abort message");
                AvHandleEvent(gsv_config,port, AvEventCecSendFeatureAbort, 0, NULL);
            }
            else
            {
                 AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgFeatureAbort:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Feature abort message");
                /*
                if ( RxContent[2] == 0x72 )
                {
        ...
        case AvEventCecMsgGivePowerStatus:
            ...
            AvHandleEvent(gsv_config,port, AvEventCecRxMessageRespond,Msg_Len, NULL);
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Give power status message");
                AvHandleEvent(gsv_config,port, AvEventCecSendPowerStatus, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgReportPowerStatus:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: get reporting power status message");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgGiveOsdName:
            ...
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Give OSD name message");
                AvHandleEvent(gsv_config,port, AvEventCecSendSetOsdName, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgGetVendorId:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Get Vendor ID");
                AvHandleEvent(gsv_config,port, AvEventCecSendDeviceVendorID, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
       ...
       case AvEventCecMsgGetCecVersion:
           /* from RxMsg AV_CEC_MSG_GET_CEC_VERSION */
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Get Cec Version");
                AvHandleEvent(gsv_config,port, AvEventCecSendCecVersion, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgRequestArcInitiation:
                 ...
                 (!AV_CEC_BROADCAST(RxContent)) && ( i == 0))
            {
                ...
                AvUapiOutputDebugMessage(gsv_config,"CEC: Request ARC Initiation message");
                ret = AvHandleEvent(gsv_config,port, AvEventCecSendInitiateARC, NULL, NULL);
                port->content.cec->EnableARC = AV_CEC_ARC_TO_INITIATE;

                if( ret != AvOk )
                {
                    AvHandleEvent(gsv_config,port, AvEventCecSendFeatureAbortReason, 0, NULL);
                }
            }
            else
            {
        ...
        case AvEventCecMsgReportArcInitiated:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Report ARC Initiated message");
                ...
                if(AudioStatus == 0)
                {
                    ...
                    AvHandleEvent(gsv_config,port, AvEventCecSendSetSystemAudioModeToTv, NULL, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgRequestArcTermination:
            ...
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Request ARC Termination message");
                ret = AvHandleEvent(gsv_config,port, AvEventCecSendTerminateARC, NULL, NULL);
                port->content.cec->EnableARC = AV_CEC_ARC_TO_TERMINATE;
                if( ret != AvOk )
                {
                    AvHandleEvent(gsv_config,port, AvEventCecSendFeatureAbortReason, 0, NULL);
        ...
        case AvEventCecMsgReportArcTerminated:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Report ARC Terminated message");
                if(AudioStatus == 1)
                {
        ...
        case AvEventCecMsgUserControlPressed:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: User Control Pressed with UI Command = 0x%x", RxContent[2]);
                switch(RxContent[2])
                {
                    case 0x41: /* Volume Up */
                        if(Cec_Tx_Audio_Status.Volume <= 100)
                            Cec_Tx_Audio_Status.Volume = Cec_Tx_Audio_Status.Volume + 10;
                        AvHandleEvent(gsv_config,port, AvEventCecSendReportAudioStatus, 0, NULL);
                        break;
        ...
        case AvEventCecMsgUserControlReleased:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: User Control Released with UI Command = 0x%x", RxContent[2]);
                switch(RxContent[2])
                {
                    case 0x41: /* Volume Up */
                        AvHandleEvent(gsv_config,port, AvEventCecSendReportAudioStatus, 0, NULL);
        ...
        case AvEventCecMsgTextViewOn:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Image/Text View On");
                AvHandleEvent(gsv_config,port, AvEventCecSendReqActiveDevice, 0, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgSystemAudioModeRequest:
            if (!AV_CEC_BROADCAST(RxContent))
            {
                ...
                {
                    InpIndex = AvCecFindRequiredInput (port, AV_CEC_PHYS_ADDR1(RxContent));
                    AvUapiOutputDebugMessage(gsv_config,"CEC: Active Source is %04x (Port %d)",
                                             AV_CEC_PHYS_ADDR1(RxContent), InpIndex);
                    AvHandleEvent(gsv_config,port, AvEventCecSendSetSystemAudioModeToAll, NULL, NULL);
                }
                else
                {
                    AvUapiOutputDebugMessage(gsv_config,"CEC: parameters number is wrong");
                    AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
                }
        ...
        case AvEventCecMsgDeckControl:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: DECK CONTROL message");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgPlay:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: PLAY message");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgGiveAudioStatus:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: give audio status");
                AvHandleEvent(gsv_config,port, AvEventCecSendReportAudioStatus, 0, NULL);
        ...
        case AvEventCecMsgGiveSystemAudioModeStatus:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: give system audio mode status");
                AvHandleEvent(gsv_config,port, AvEventCecSendReportSystemAudioModeStatus, 0, NULL);
        ...
        case AvEventCecMsgRequestShortAudioDescriptor:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: give short audio descriptor");
                AvHandleEvent(gsv_config,port, AvEventCecSendReportShortAudioDecriptor, 0, NULL);
        ...
        case AvEventCecMsgSystemAudioModeStatus:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvHandleEvent(gsv_config,port, AvEventCecReceiveSystemAudioModeStatus, 0, NULL);
                /* ReceiveSystemAudioModeStatus(RxContent); */
                AvUapiOutputDebugMessage(gsv_config,"CEC: receiving message about system audio mode status");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgSetSystemAudioMode:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                ret = AvHandleEvent(gsv_config,port, AvEventCecReceiveSetSystemAudioMode, 0, NULL);
                ...
                */
                AvUapiOutputDebugMessage(gsv_config,"CEC receiving message about setting system audio mode");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgReportAudioStatus:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvHandleEvent(gsv_config,port, AvEventCecReceiveAudioStatus, 0, NULL);
                /* ReceiveAudioStatus(RxContent); */
                AvUapiOutputDebugMessage(gsv_config,"CEC: receiving message about audio status");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgSetAudioRate:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvHandleEvent(gsv_config,port, AvEventCecReceiveSetAudioRate, 0, NULL);
                /* ReceiveSetAudioRate(RxContent); */
                AvUapiOutputDebugMessage(gsv_config,"CEC: receiving message about audio rate");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgReportShortAudioDescriptor:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: receiving message about short audio descriptor");
                AvHandleEvent(gsv_config,port, AvEventCecReceiveShortAudioDescriptor, 0, NULL);
        ...
        case AvEventCecMsgReportPhyAddr:
            ...
            if (AV_CEC_BROADCAST(RxContent))
            {
                ...
                */
                ...
                AvUapiOutputDebugMessage(gsv_config,"CEC: source %x reports phy addr",ActiveSource);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgMenuRequest:
            ...
            if (!AV_CEC_BROADCAST(RxContent))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Receive menu request %x", RxContent[2]);
                AvHandleEvent(gsv_config,port, AvEventCecSendMenuStatus, NULL, NULL);
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgRequestActiveSource:
            ...
            {
                AvHandleEvent(gsv_config,port, AvEventCecSendActiveSource, NULL, NULL);
            }
            else
            {
                 AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
        ...
        case AvEventCecMsgSetMenuLanguage:
            ...
            if(AV_CEC_BROADCAST(RxContent)&&
               (port->content.cec->LogAddr == AV_CEC_TV_LOG_ADDRESS))
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: Set menu language");
            }
            else
            {
                AvUapiOutputDebugMessage(gsv_config,"CEC: ignore");
...
AvRet AvEdidPortManage(GSV_CONFIG *gsv_config,AvPort *RxPort)
{
    ...
    AvMemset(gsv_config->InEdid, 0, 256);
    AvMemset(gsv_config->SinkEdid, 0, 256);
    AvMemset(gsv_config->OutEdid, 0, 256);
...
//    AvEdidCeaParamRemove =  0;
    ...
    while(KfunFindVideoNextTxEnd(RxPort, &PrevPort, &CurrentPort) == AvOk)
    {
           ...
           (CurrentPort->content.tx->IgnoreEdidError == 0))
        {
            ...
            AvUapiOutputDebugMessage(gsv_config,"Rx %d Edid Manage: Read Tx %d EDID",RxPort->index+1, CurrentPort->index-3);
            /* find the next Edid Ram Ptr */
            if(SinkNumber == 1)
                KfunTxReadEdid(gsv_config,CurrentPort,gsv_config->InEdid);
            else
            {
                KfunTxReadEdid(gsv_config,CurrentPort,gsv_config->SinkEdid);
                /* Merge Edids */
                AvEdidFuncStructInit(&gsv_config->DevEdidReg);
                AvMemset(gsv_config->OutEdid, 0, 256);
                AvEdidFuncVesaProcess(&gsv_config->DevEdidReg,gsv_config->InEdid,gsv_config->SinkEdid,gsv_config->OutEdid);
                AvEdidFuncCeaProcess(gsv_config,&gsv_config->DevEdidReg,gsv_config->InEdid,gsv_config->SinkEdid,gsv_config->OutEdid);
                AvMemcpy(gsv_config->InEdid,gsv_config->OutEdid,256);
                AvMemset(gsv_config->SinkEdid, 0, 256);
            }
        }
    ...
    //AvMemcpy(gsv_config->OutEdid+0,(void*)(&AvDefaultEDID[0]),256);
    AvEdidFuncCheckSum(gsv_config->OutEdid);

    /* 4. Find Spa Location and generate its own SPA for source */
    SpaLocation = KfunFindCecSPAFromEdid(gsv_config,gsv_config->OutEdid, SpaValue);
    KfunGenerateSourceSpa(RxPort, SpaValue, 0);

    AvUapiOutputDebugMessage(gsv_config,"Rx %d Generated Edid Start: ",RxPort->index+1);
    for(i=0;i<16;i++)
    {
        AvUapiOutputDebugMessage(gsv_config," %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
                                 gsv_config->OutEdid[i*16+0],gsv_config->OutEdid[i*16+1],gsv_config->OutEdid[i*16+2],gsv_config->OutEdid[i*16+3],
                                 gsv_config->OutEdid[i*16+4],gsv_config->OutEdid[i*16+5],gsv_config->OutEdid[i*16+6],gsv_config->OutEdid[i*16+7],
                                 gsv_config->OutEdid[i*16+8],gsv_config->OutEdid[i*16+9],gsv_config->OutEdid[i*16+10],gsv_config->OutEdid[i*16+11],
                                 gsv_config->OutEdid[i*16+12],gsv_config->OutEdid[i*16+13],gsv_config->OutEdid[i*16+14],gsv_config->OutEdid[i*16+15]);
    }

    /* 5. write Edid, set rx->EdidStatus to AV_EDID_NEEDUPDATE */
    KfunRxWriteEdid(gsv_config,RxPort,gsv_config->OutEdid, SpaLocation, SpaValue);
...
void AvPortSetRouting(GSV_CONFIG *gsv_config,AvPort *RxPort, AvPort *TxPort, AvPort *ColorPort, AvPort *ScalerPort, uint8 TxConnectStyle)
{
    uint8 MessageFlag = 0;
    switch(TxConnectStyle)
    {
        ...
        case ROUTE_7_V_T:
            ...
            {
                MessageFlag = 1;
                AvApiConnectPort(gsv_config,RxPort,    TxPort,     AvConnectVideo);
            }
            if(MessageFlag == 1)
            {
                if(TxConnectStyle == ROUTE_1_R_T)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-Tx%d",RxPort->index+1, TxPort->index-3);
                else if(TxConnectStyle == ROUTE_4_R_V)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-LogicVideoTx",RxPort->index+1);
                else
                    AvUapiOutputDebugMessage(gsv_config,"New Route: LogicVideoRx-Tx%d",TxPort->index-3);
            }
            break;
        case ROUTE_2_R_C_T:
        case ROUTE_5_R_C_V:
        case ROUTE_8_V_C_T:
            ...
            {
                MessageFlag = 1;
                AvApiConnectPort(gsv_config,RxPort,    ColorPort,  AvConnectVideo);
            }
            if(TxPort->content.RouteVideoFromPort != (struct AvPort*)ColorPort)
            {
                MessageFlag = 1;
                AvApiConnectPort(gsv_config,ColorPort, TxPort,     AvConnectVideo);
            }
            if(MessageFlag == 1)
            {
                if(TxConnectStyle == ROUTE_2_R_C_T)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-Color-Tx%d",RxPort->index+1, TxPort->index-3);
                else if(TxConnectStyle == ROUTE_5_R_C_V)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-Color-LogicVideoTx",RxPort->index+1);
                else
                    AvUapiOutputDebugMessage(gsv_config,"New Route: LogicVideoRx-Color-Tx%d",TxPort->index-3);
            }
            break;
        case ROUTE_3_R_S_T:
        case ROUTE_6_R_S_V:
        case ROUTE_9_V_S_T:
            ...
            {
                MessageFlag = 1;
                AvApiConnectPort(gsv_config,RxPort,    ScalerPort, AvConnectVideo);
            }
            if(TxPort->content.RouteVideoFromPort != (struct AvPort*)ScalerPort)
            {
                MessageFlag = 1;
                AvApiConnectPort(gsv_config,ScalerPort, TxPort,     AvConnectVideo);
            }
            if(MessageFlag == 1)
            {
                if(TxConnectStyle == ROUTE_3_R_S_T)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-Scaler-Tx%d",RxPort->index+1, TxPort->index-3);
                else if(TxConnectStyle == ROUTE_6_R_S_V)
                    AvUapiOutputDebugMessage(gsv_config,"New Route: Rx%d-Scaler-LogicVideoTx",RxPort->index+1);
                else
                    AvUapiOutputDebugMessage(gsv_config,"New Route: LogicVideoRx-Scaler-Tx%d", TxPort->index-3);
            }
            break;
    }
}

AvRet AvPortConnectUpdate(GSV_CONFIG *gsv_config,AvDevice *Device)
{
    AvPort *TxPort;
    AvPort *RxPort;
    AvPort *VideoTxPort;
    ...
    if(KfunFindVideoRxFront(TxPort, &RxPort) == AvOk)
    {
        if(RxPort->type == HdmiRx)
        {
            if(gsv_config->LogicOutputSel == 0)
            {
                RxPort = VideoRxPort;
                AvApiConnectPort(gsv_config,VideoRxPort, TxPort, AvConnectVideo);
                AvApiConnectPort(gsv_config,AudioRxPort, TxPort, AvConnectAudio);
        ...
        else if(RxPort->type == LogicVideoRx)
        {
            ...
            {
                RxPort = (AvPort*)Device->port;
                AvApiConnectPort(gsv_config,RxPort, TxPort,      AvConnectAV);
                AvApiConnectPort(gsv_config,RxPort, VideoTxPort, AvConnectVideo);
                AvApiConnectPort(gsv_config,RxPort, AudioTxPort, AvConnectAudio);
    ...
    if(gsv_config->LogicOutputSel == 1)
    {
        ...
        if((TxCurrentStyle != TxConnectStyle) && (TxConnectStyle != ROUTE_NO_CONNECT))
            AvPortSetRouting(gsv_config,RxPort, VideoTxPort, ColorPort, ScalerPort, TxConnectStyle);
    ...
    else
    {
        ...
        if((TxCurrentStyle != TxConnectStyle) && (TxConnectStyle != ROUTE_NO_CONNECT))
            AvPortSetRouting(gsv_config,VideoRxPort, TxPort, ColorPort, ScalerPort, TxConnectStyle);
    }
